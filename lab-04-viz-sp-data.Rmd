...
***There are `r nrow(dennys)` rows and there are `r ncol(dennys)` columns. Each row represents the data for a Denny's restaurant. The variables are `r names(dennys)`.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
***There are `r nrow(laquinta)` rows and there are `r ncol(laquinta)` columns. Each row represents the data for a La Quinta hotel. The variables are `r names(laquinta)`.***
...
***There are no Denny's locations outside of the US according to our data.***
...
***The state with the most Denny's locations is California with 403 locations, the state with the least Denny's locations is Delaware with 1 location. This is not very surprising to me as Denny's is a 24-hour restaurant and California is a state that is almost always busy so 24-hour restaurants would thrive.***
...
***The state with the most La Quinta locations is Texas with 237 locations, the state with the least La Quinta locations is Maine with 1 location. This surprises me, as Texas does not have the highest amount of Denny's restaurants but has lots of La Quinta hotels. It is also not as surprising considering that Texas has two big, well-known cities: Austin and Houston.***
